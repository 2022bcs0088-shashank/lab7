pipeline {
    ...
    stages {
        ...
        stage('Send Invalid Request') {
            steps {
                script {
                    def status = sh(script: "curl -s -o /dev/null -w '%{http_code}' -X POST http://172.17.0.1:8000/predict -H 'Content-Type: application/json' -d '{\"bad_field\": 0}'", returnStdout: true).trim()
